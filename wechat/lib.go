...
func Request(input map[string]interface{}) (result interface{}, err error) {
	...
	if input["body_params"] != nil {
		...
		if requestType == "xml" {
			bodyStr = input["body_params"].([]byte)
		}
	}
	...
	if input["success"] != nil {
		return input["success"].(func(response *http.Response) (res interface{}, err error))(response)
	}
 ...
 * @date 2020-04-14 22:10:18
 */
func GetRequestType(input map[string]interface{}) (requestType string) {

	if input["headers"] == nil {
		return "json"
	}
	if input["headers"].(map[string]interface{})["Content-Type"] == nil {
		return "json"
	}
	contentType := input["headers"].(map[string]interface{})["Content-Type"].(string)
 ...
 * @date 2020-04-14 22:10:18
 */
func GetResponseType(response *http.Response) (responseType string) {
...
func OrderParam(p map[string]string, bizKey string) (returnStr string) {
	...
	for _, k := range keys {
		...
		res = append(res, k+"="+p[k])
	}
	returnStr = strings.Join(res, "&")
	returnStr += bizKey
	return returnStr
}
...
func RandomStr(length int) string {
	...
	for i := 0; i < length; i++ {
		result = append(result, iBytes[r.Intn(len(iBytes))])
	}
